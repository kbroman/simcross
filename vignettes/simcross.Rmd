...
```{r ail_few_pairs}
tab2 <- sim_ail_pedigree(ngen=8, npairs=3, nkids_per=50)
xodat2 <- sim_from_pedigree(tab2, L=100)
prop_het2 <- sapply(lapply(xodat2, where_het), function(a) sum(a[,2]-a[,1])/100)
ave_prop_het2 <- tapply(prop_het2, tab2[,"gen"], mean)
...
For crosses with more than two founders, the genotypes are replaced
with integers representing binary codes: the genotype
(_m_,_n_) will be assigned the integer $2^{m-1} + 2^{n-1}$.
